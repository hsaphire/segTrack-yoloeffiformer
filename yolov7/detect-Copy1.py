...
from utils.visualize import plot_tracking,plot_tracking_route
...
def detect(save_img=False):
    ...
    save_path2 = os.path.join("video_save","camera.mp4")
    ...
    vid_writer2 = cv2.VideoWriter(
        save_path2, cv2.VideoWriter_fourcc(*"mp4v"), 30, (640, 384)
    )
    t0 = time.time()
    count = 0
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                #rint(obj_conf.shape)
                #rint("2513631453",det[:,:4].shape)
                #print("2513631453",det[:,:4])
...
#######################################################################################################
                ...
                for t in online_targets:
                    ...
                    if tlwh[2] * tlwh[3] > opt.min_box_area and not vertical:
                        ...
                        results.append(
                            f"{count},{tid},{tlwh[0]:.2f},{tlwh[1]:.2f},{tlwh[2]:.2f},{tlwh[3]:.2f},{t.score:.2f},-1,-1,-1\n"
                        )
                #print(type(results))
                #print(len(results))
                ...
                #out = cv2.cvtColor(np.array((img[0].permute(1,2,0)),cv2.COLOR_BGR2RGB))
                #print(img[0].numpy())
            ...
            try:
                ...
                pass
            img_save = os.path.join(f"img_save/{source}")
            #print(online_im[0].shape)

            #out = cv2.cvtColor(np.transpose(online_im[0],(1,2,0)),cv2.COLOR_BGR2RGB)
...
##################################################################################################################
        ...
        if len(det):
            ...
            # what i want (batch_size, num_objects, x_center, y_center, width, height, obj_conf, category)
            # Write results
            for *xyxy, conf, cls in reversed(det): #det is not model output
                #print(int(cls))
    ...
    #def plot_black_route(obj_id,center_point):
