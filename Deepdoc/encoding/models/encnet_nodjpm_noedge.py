...
class EncNet(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=True,
        ...
        '''
        self.edge_4 = nn.Sequential(nn.Conv2d(48,  self.w, 3, padding=1, bias=False),
                                   norm_layer(self.w),
        ...
        self.edge_6 = nn.Sequential(nn.Conv2d(40, self.w, 3, padding=1, bias=False),
                                   ...
                                   nn.ReLU(inplace=True))
        '''    
        self.edge_4 = nn.Sequential(MobileNetConv2d(48,  self.w),
                                   norm_layer(self.w),
                                   nn.ReLU(inplace=True))
    ...
    def forward(self, x):
        ...
        #y3 = F.interpolate(self.edge_7(features[3]),imsize,**self._up_kwargs)
        
        #yres = self.c(torch.cat([y, y1, y2,y3], 1))
...
class EncModule(nn.Module):
    def __init__(self, in_channels, nclass, ncodes=32, se_loss=True, norm_layer=None):
        ...
        self.encoding = nn.Sequential(
            MobileNetConv2d(in_channels, in_channels)#, 1, bias=False),3/29
...
class EncHead(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, se_loss=True, jpu=True, lateral=False,
        ...
        self.conv5 = nn.Sequential(nn.Conv2d(in_channels[-1], 512, 1, bias=False),
                     ...
                     nn.Sequential(MobileNetConv2d(in_channels[-1], 512,) #3, padding=1, bias=False),#3/29
                                   norm_layer(512),
                                   nn.ReLU(inplace=True))
        if lateral:
